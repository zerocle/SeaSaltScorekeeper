  ...
  switch (key) {
    ...
    case "penguins":
      return (
        ...
        (bd.multiplierCards.penguin ? bd.collectorCards.penguins * 2 : 0)
      );
    case "sailors":
      return (
...
export function CompactScoreTable({
  ...
  const [activeModal, setActiveModal] = useState<RowKey | string | null>(null);
  ...
  const applyMermaidColorValues = (mermaidIndex: number, values: number[]) => {
  ...
  const renderCardRow = (config: RowConfig) => {
    ...
    return (
      <View key={config.key}>
        <View style={[styles.row, isOverAllocated && styles.rowOverAllocated]}>
          <TouchableOpacity
          ...
          <View style={styles.rowRight}>
            {rowScores.map((score, i) => {
              ...
              const hasCardsButZero = score === 0 && cardCount > 0;
              return (
                <TouchableOpacity
                  key={i}
                  style={[
                    styles.valuePill,
                    manualScores[i] !== null && styles.valuePillDimmed,
                  ]}
                  onPress={() => openModalForPlayer(config.key, i)}
                ...
                >
                  <Text
                    style={[
                      ...
                      score > 0 && styles.valuePillNumberActive,
                      hasCardsButZero && styles.valuePillNumberZeroWithCards,
                    ]}
                  >
                    {hasCardsButZero ? `0 (${cardCount})` : score}
                  </Text>
                  {config.multiplier && multiplierActiveIndex === i && (
        ...
        <RowEntryModal
          ...
          onConfirm={(vals, mpi) =>
            applyRowValues(config.key, vals, config.multiplier?.field, mpi)
          }
  ...
  const showCrossPlayerErrors = submitAttempted && crossPlayerErrors.length > 0;
    ...
    <View style={styles.container}>
      ...
      {Array.from({ length: maxMermaidCount }, (_, mIdx) => {
        ...
        const hasRow = players.some(
          (_, pIdx) => mIdx < (breakdowns[pIdx]?.mermaids.length ?? 0),
        );
          ...
          <View key={modalKey}>
            <View style={styles.row}>
              <TouchableOpacity
                ...
                accessibilityRole="button"
              >
                <Text style={styles.rowLabel}>M{mIdx + 1} Color</Text>
              </TouchableOpacity>
              <View style={styles.rowRight}>
                {players.map((_, pIdx) => {
                  const hasMermaid =
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.valuePill,
                        manualScores[pIdx] !== null && styles.valuePillDimmed,
                      ]}
                      onPress={() => openModalForPlayer(modalKey, pIdx)}
                    ...
                    >
                      <Text
                        style={[
                          styles.valuePillNumber,
                          hasMermaid && v > 0 && styles.valuePillNumberActive,
                          !hasMermaid && styles.valuePillNumberDash,
                        ]}
                      >
                        {hasMermaid ? v : "—"}
                      </Text>
            ...
            <RowEntryModal
              ...
              onConfirm={(vals) => applyMermaidColorValues(mIdx, vals)}
              onClose={() => setActiveModal(null)}
            />
          </View>
        );
        ...
        <View style={styles.scoreValues}>
          {players.map((player, index) => {
            ...
            return (
              <TouchableOpacity
                key={index}
                style={[styles.scorePill, isManual && styles.scorePillManual]}
                onPress={() => handleScorePillPress(index)}
                accessibilityLabel={`Set total score for ${player.name}`}
                accessibilityRole="button"
              >
                <Text style={styles.scorePillValue}>{displayScore}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
          ...
          >
            ...
            >
              ...
              {manualScoreModal.hasRowData &&
                manualScores[manualScoreModal.playerIndex] === null && (
                  <View style={styles.manualScoreWarning}>
                    <Text style={styles.manualScoreWarningText}>
                      ⚠ Row entries exist — confirming will override the
                      calculated score for this player only.
                    </Text>
                  </View>
                )}
              ...
              <View style={styles.numpad}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setManualScoreModal((prev) => {
                        if (!prev) return prev;
                        if (prev.inputText === "0" || prev.inputText === "")
                          return {
                            ...prev,
                            inputText: String(n),
                          };
                        if (prev.inputText.length >= 4) return prev;
                        return {
                          ...prev,
                          inputText: prev.inputText + String(n),
                        };
                      })
                    }
                ...
                <TouchableOpacity
                  style={[styles.numpadKey, styles.numpadKeyAlt]}
                  onPress={() =>
                    setManualScoreModal((prev) =>
                      prev ? { ...prev, inputText: "" } : prev,
                    )
                  }
                  accessibilityLabel="Clear"
                  accessibilityRole="button"
                >
                  <Text style={[styles.numpadKeyText, styles.numpadKeyAltText]}>
                    C
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setManualScoreModal((prev) => {
                      if (!prev) return prev;
                      if (prev.inputText === "" || prev.inputText === "0")
                        return prev;
              ...
              <View style={styles.manualScoreActions}>
                <TouchableOpacity
                  ...
                  accessibilityRole="button"
                >
                  <Text style={styles.manualScoreCancelText}>Cancel</Text>
                </TouchableOpacity>
                {manualScores[manualScoreModal.playerIndex] !== null && (
                  <TouchableOpacity
                    ...
                    accessibilityRole="button"
                  >
                    <Text style={styles.manualScoreCancelText}>Use Rows</Text>
                  </TouchableOpacity>
                )}
                <TouchableOpacity
                  ...
                  accessibilityRole="button"
                >
                  <Text style={styles.manualScoreConfirmText}>Done</Text>
                </TouchableOpacity>
              </View>
            </Pressable>
          </Pressable>
      ...
      {showValidation && (
        <View style={styles.validationBox}>
          {validationErrors.map((errs, pIdx) =>
            errs.map((err, i) => (
              <Text key={`${pIdx}-${i}`} style={styles.validationError}>
