  ...
  return (
    <View style={styles.playerCell}>
      <Text style={[styles.cellText, highlight && styles.highlightCell]}>
        {value}
        {suffix ? <Text style={styles.multiplierSuffix}> {suffix}</Text> : null}
      </Text>
    </View>
  );
}
      ...
      <Pressable style={styles.overlay} onPress={onClose}>
        <Pressable style={styles.content} onPress={(e) => e.stopPropagation()}>
          <View style={styles.header}>
            <Text style={styles.title}>Round {data.roundNumber}</Text>
            <TouchableOpacity
              onPress={onClose}
              hitSlop={12}
          ...
          <ScrollView style={styles.body}>
            ...
            <View style={styles.tableRow}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(Math.floor(p.breakdown.duoCards.crabs / 2))}
                />
              ))}
            </View>
            <View style={styles.tableRow}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(Math.floor(p.breakdown.duoCards.boats / 2))}
                  suffix={
            ...
            <View style={styles.tableRow}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(Math.floor(p.breakdown.duoCards.fish / 2))}
                  suffix={
            ...
            <View style={styles.tableRow}>
              <View style={styles.labelCell}>
                <Text style={styles.labelText}>Swimmer+Shark</Text>
              </View>
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(p.breakdown.duoCards.swimmerSharkCombos)}
                />
              ))}
            </View>
            ...
            <View style={styles.tableRow}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(p.breakdown.collectorCards.shells)}
                />
              ))}
            </View>
            <View style={styles.tableRow}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(p.breakdown.collectorCards.octopus)}
                />
              ))}
            </View>
            <View style={styles.tableRow}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(p.breakdown.collectorCards.penguins)}
                  suffix={
            ...
            <View style={styles.tableRow}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  key={i}
                  value={String(p.breakdown.collectorCards.sailors)}
                  suffix={
              ...
              <>
                ...
                <View style={styles.tableRow}>
                  ...
                  {data.players.map((p, i) => (
                    <ReadOnlyCell
                      key={i}
                      value={String(p.breakdown.mermaids.length)}
                    />
                  ))}
                </View>
                  ...
                  <View key={mIdx} style={styles.tableRow}>
                    ...
                    {data.players.map((p, i) => (
                      <ReadOnlyCell
                        key={i}
                        value={
                          mIdx < p.breakdown.mermaids.length
                            ? String(p.breakdown.mermaids[mIdx].colorCount)
                            : "—"
                        }
                      />
                    ))}
            ...
            <View style={[styles.tableRow, styles.scoreRow]}>
              ...
              {data.players.map((p, i) => (
                <ReadOnlyCell
                  ...
                  highlight={p.totalScore === maxScore && maxScore > 0}
                />
              ))}
            </View>
            ...
            {data.lcData && (
              <View style={styles.lcSection}>
                <View style={styles.lcLabelRow}>
                  <DiceIcon size={14} color={colors.textPrimary} />
                  <Text style={styles.lcLabel}> Last Chance</Text>
                </View>
                <Text style={styles.lcDetail}>
                  Caller: {data.getPlayerName(data.lcData.callerIndex)}
                </Text>
                <Text
                  style={[
                    styles.lcDetail,
                ...
                >
                  {data.lcData.outcome === "won" ? "✓ Won" : "✗ Lost"}
                </Text>
              </View>
            )}
          </ScrollView>
